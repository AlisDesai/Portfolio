...
import { ServicesHeroCollageTile } from "@/components/features/services-hero/ServicesHeroCollageTile";

const EASE_PREMIUM = [0.16, 1, 0.3, 1] as const;

interface ServicesHeroCollageProps {
  reduceMotion: boolean;
}

// Reveal delay is grouped (not per-tile) so the collage builds itself in small
// clusters — matches the same reveal rhythm used on the Home Hero collage.
const COLLAGE_START = 0.1;
const GROUP_SIZE = 4;
const GROUP_STEP = 0.12;
const GROUP_JITTER = 0.03;

export function ServicesHeroCollage({ reduceMotion }: ServicesHeroCollageProps) {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      {COLLAGE_TILES.map((tile, index) => {
        ...
        return (
          ...
          >
            <ServicesHeroCollageTile tile={tile} blurClassName={blur} />
          </motion.div>
        );
      })}
    </div>
